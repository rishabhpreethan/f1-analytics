...
const hex2rgb = (h) => {
  ...
  return [parseInt(n.slice(0, 2), 16), parseInt(n.slice(2, 4), 16), parseInt(n.slice(4, 6), 16)].map((v) => v / 255);
};
const rgb2hex = (rgb) =>
...
function de2000(l1, l2) {
  ...
  else { const d = Math.abs(hp1 - hp2); Hbp = d > 180 ? (hp1 + hp2 + 360) / 2 : (hp1 + hp2) / 2; if (d > 180 && hp1 + hp2 >= 360) Hbp -= 360; }
...
function inv3(M) {
  ...
  const A = e * i - f2 * h, B = -(d * i - f2 * g), C = d * h - e * g;
...
const minCVD = (h1, h2) => Math.min(...['protan', 'deutan', 'tritan'].map((k) => cvdDE(h1, h2, k).worse));

/* ================================================================ DATA */
const BRAND = {
  alpine: '#00A1E8', aston_martin: '#229971', audi: '#FF2D00', cadillac: '#AAAAAD',
  ferrari: '#ED1131', haas: '#9C9FA2', mclaren: '#F47600', mercedes: '#00D7B6',
  rb: '#6C98FF', red_bull: '#4781D7', sauber: '#01C00E', williams: '#1868DB',
};
const SURF = {
  light: { sunken: '#EFF1F5', canvas: '#F7F8FB', raised: '#FFFFFF', overlay: '#FFFFFF', inkPrimary: '#1B1E24', inkInverse: '#FFFFFF' },
  dark: { sunken: '#08090C', canvas: '#0E0F13', raised: '#1A1C20', overlay: '#23252A', inkPrimary: '#F5F7F9', inkInverse: '#0E0F13' },
};
const TIMING = {
...
function calibration() {
  ...
  console.log(`Cadillac<->Haas normal dE      : ${n(dE(BRAND.cadillac, BRAND.haas))}   (recorded 3.82)`);
  console.log(`Haas OkLCh C                   : ${n(oklch(BRAND.haas).C, 4)}  (recorded 0.0056)`);
  console.log(`Cadillac OkLCh C               : ${n(oklch(BRAND.cadillac).C, 4)}  (recorded 0.0043)`);
  console.log(`Mercedes OkLCh L               : ${n(oklch(BRAND.mercedes).L, 3)}   (recorded 0.786)`);
  const rbAl = cvdDE(BRAND.rb, BRAND.alpine, 'deutan');
  console.log(`RB<->Alpine deutan dE          : Vienot ${n(rbAl.vienot)} / Machado ${n(rbAl.machado)}  (recorded 3.17 / 2.90)`);
  const t = cvdDE(BRAND.rb, BRAND.alpine, 'tritan');
  console.log(`RB<->Alpine tritan dE          : Vienot ${n(t.vienot)} / Machado ${n(t.machado)}  (recorded 4.62 / 3.49)`);
  console.log('\n-- V-3 spot check: timing semantics, light --');
  console.log(`green<->yellow deutan          : ${n(cvdDE(TIMING.light.green, TIMING.light.yellow, 'deutan').worse)}  (recorded 6.9)`);
  console.log(`purple<->yellow tritan         : ${n(cvdDE(TIMING.light.purple, TIMING.light.yellow, 'tritan').worse)}  (recorded 7.2)`);
  console.log(`min CVD dE, dark timing set    : ${n(Math.min(minCVD(TIMING.dark.green, TIMING.dark.yellow), minCVD(TIMING.dark.purple, TIMING.dark.yellow), minCVD(TIMING.dark.purple, TIMING.dark.green)))}  (recorded 9.2)`);
  console.log('\n-- V-2 spot check: neutrals --');
  console.log(`ink-tertiary on sunken, light  : ${n(contrast('#6A6D74', SURF.light.sunken))}:1  (recorded 4.58)`);
  console.log(`border-control on sunken, light: ${n(contrast('#878B92', SURF.light.sunken))}:1  (recorded 3.03)`);
  console.log(`ink-tertiary on sunken, dark   : ${n(contrast('#86898F', SURF.dark.sunken))}:1  (recorded 5.68)`);
}
...
function hueScan(theme) {
  ...
  for (let h = 0; h < 360; h += 5) {
    ...
    if (!best) { rows.push({ h, hex: '—', C: 0, minRes: 0, minStat: 0, minBrand: 0 }); continue; }
  ...
  console.log('  h   hex        L     C      minCVD-vs-timing  minNormal-vs-timing  minCVD-vs-status  minNormal-vs-brand');
  for (const r of rows) {
    if (r.hex === '—') { console.log(` ${String(r.h).padStart(3)}  (no in-gamut candidate)`); continue; }
    console.log(
...
function ramp(hue) {
  ...
  for (const theme of ['light', 'dark']) {
    ...
    let strong = null;
    for (let L = 0.20; L <= 0.95; L += 0.002) {
      const { hex, C } = lch2hex(L, 0.40, H);
      if (contrast(hex, S.raised) < 4.5 || contrast(hex, S.canvas) < 4.5 || contrast(hex, S.sunken) < 4.5) continue;
    ...
    for (let L = 0.20; L <= 0.95; L += 0.002) {
      ...
      if (!base || C > base.C) base = { hex, C, L, on: onWhite >= onDark ? '#FFFFFF' : SURF.light.inkPrimary, onRatio: best };
    }
    // accent-mark: >=3:1 on raised AND sunken, max chroma, inside the plotting band
    const band = theme === 'light' ? [0.40, 0.72] : [0.55, 0.88];
    let mark = null;
    ...
    for (const [k, v] of Object.entries(rows)) {
      if (!v) { console.log(`${k.padEnd(8)} : NO SOLUTION`); continue; }
      console.log(`${k.padEnd(8)} : ${v.hex}  L ${n(v.L, 3)}  C ${n(v.C, 3)}  ` +
    ...
    for (const [name, hex] of [['timing-purple', TIMING[theme].purple], ['timing-green', TIMING[theme].green], ['timing-yellow', TIMING[theme].yellow],
      ['status-info', STATUS[theme].info], ['status-good', STATUS[theme].good], ['status-caution', STATUS[theme].caution], ['status-critical', STATUS[theme].critical]]) {
      const a = rows.strong.hex, b = rows.mark.hex;
      console.log(`    ${name.padEnd(16)} strong: normal ${n(dE(a, hex)).padStart(6)}  minCVD ${n(minCVD(a, hex)).padStart(6)}   |   mark: normal ${n(dE(b, hex)).padStart(6)}  minCVD ${n(minCVD(b, hex)).padStart(6)}`);
    }
    console.log('  separation audit (accent-mark vs the 12 brand colours, normal vision):');
    const bl = Object.entries(BRAND).map(([t, hex]) => `${t} ${n(dE(rows.mark.hex, hex), 1)}`).join('  ');
    console.log('    ' + bl);
    console.log(`    worst brand pair: ${Object.entries(BRAND).map(([t, hex]) => [t, dE(rows.mark.hex, hex)]).sort((x, y) => x[1] - y[1])[0].join(' = ')}`);
  }
}
...
function check(list) {
  ...
  for (const [name, hex] of set) {
    const o = oklch(hex);
    console.log(`${name.padEnd(20)} ${hex}  L ${n(o.L, 3)} C ${n(o.C, 3)} h ${n(o.h, 1)}  ` +
  ...
  for (let i = 0; i < set.length; i++) for (let j = i + 1; j < set.length; j++) {
    console.log(`  ${set[i][0]} <-> ${set[j][0]}: ${n(dE(set[i][1], set[j][1]))} / ${n(minCVD(set[i][1], set[j][1]))}`);
  }
}
...
function steps(hue) {
  const H = Number(hue);
  const Ls = [0.97, 0.94, 0.88, 0.80, 0.72, 0.658, 0.568, 0.47, 0.36, 0.26, 0.18];
  const names = ['50', '100', '200', '300', '400', '500', '600', '700', '800', '850', '900'];
  console.log(`\n=== SIGNAL RAMP, OkLCh hue ${H} ===`);
  console.log('step  hex       L      C      | LIGHT raised/canvas/sunken | DARK raised/canvas/sunken | vs #FFF | vs #1B1E24 | vs #0E0F13');
  const out = {};
  Ls.forEach((L, i) => {
    const { hex, C } = lch2hex(L, 0.40, H);
    out[names[i]] = hex;
    const lp = [SURF.light.raised, SURF.light.canvas, SURF.light.sunken].map((s) => n(contrast(hex, s))).join('/');
    const dp = [SURF.dark.raised, SURF.dark.canvas, SURF.dark.sunken].map((s) => n(contrast(hex, s))).join('/');
    console.log(`${names[i].padEnd(5)} ${hex}  ${n(L, 3)}  ${n(C, 3)}  | ${lp.padEnd(26)} | ${dp.padEnd(25)} | ${n(contrast(hex, '#FFFFFF')).padStart(5)}   | ${n(contrast(hex, '#1B1E24')).padStart(5)}      | ${n(contrast(hex, '#0E0F13')).padStart(5)}`);
  });
  console.log('\nCVD audit for the two load-bearing steps vs reds (the plausible confusion):');
  for (const step of ['500', '600']) {
    for (const [nm, hx] of [['ferrari', BRAND.ferrari], ['audi', BRAND.audi], ['critical-light', STATUS.light.critical], ['critical-dark', STATUS.dark.critical]]) {
      console.log(`  signal-${step} <-> ${nm.padEnd(14)} normal ${n(dE(out[step], hx)).padStart(6)}  protan ${n(cvdDE(out[step], hx, 'protan').worse).padStart(5)}  deutan ${n(cvdDE(out[step], hx, 'deutan').worse).padStart(5)}  tritan ${n(cvdDE(out[step], hx, 'tritan').worse).padStart(5)}`);
    }
  }
  console.log('\nnormal-vision separation of signal-500/600 from every reserved semantic (floor 15):');
  for (const theme of ['light', 'dark']) {
    for (const [nm, hx] of [...Object.entries(TIMING[theme]).map(([k, v]) => [`timing-${k}`, v]), ...Object.entries(STATUS[theme]).map(([k, v]) => [`status-${k}`, v])]) {
      console.log(`  ${theme} ${nm.padEnd(18)} vs 500 ${n(dE(out['500'], hx)).padStart(6)}   vs 600 ${n(dE(out['600'], hx)).padStart(6)}`);
    }
  }
  console.log('\nlightness band check (plot marks: light 0.40-0.72, dark 0.55-0.88), chroma floor 0.05:');
  for (const s of ['400', '500', '600']) { const o = oklch(out[s]); console.log(`  signal-${s} L ${n(o.L, 3)} C ${n(o.C, 3)} h ${n(o.h, 1)}`); }
}
...
function pairs() {
  const P = [
    ...
    ['DARK   accent-wash #46002B vs surface-raised (visible field)', '#46002B', '#1A1C20', 1.10],
    ['DARK   accent-wash #370021 vs surface-raised (visible field)', '#370021', '#1A1C20', 1.10],
  ...
  for (const h of ['#FEF1F7', '#FFE2EE', '#FFC4DE', '#FF98CA', '#FF61B7', '#FE02A9', '#D1018A', '#A2006A', '#700148', '#46002B', '#370021', '#270016']) {
    const o = oklch(h); console.log(`  ${h}  L ${n(o.L, 3)}  C ${n(o.C, 3)}  h ${n(o.h, 1)}`);
  }
}
if (mode === 'pairs') pairs();
...
function fix() {
  ...
  for (const L of [0.28, 0.30, 0.32, 0.34, 0.36]) {
    const { hex, C } = lch2hex(L, 0.40, 350);
    console.log(`  L ${n(L,2)} ${hex} C ${n(C,3)}  vs raised#1A1C20 ${n(contrast(hex,'#1A1C20'))}:1  vs canvas#0E0F13 ${n(contrast(hex,'#0E0F13'))}:1` +
  ...
  for (const [k, v] of Object.entries(TIMING_WASH.dark)) console.log(`  ${k.padEnd(7)} ${v}  vs raised ${n(contrast(v,'#1A1C20'))}:1   ink ${TIMING.dark[k]} on it ${n(contrast(TIMING.dark[k], v))}:1`);
  console.log('\nprecedent — existing light timing washes vs surface-raised:');
  for (const [k, v] of Object.entries(TIMING_WASH.light)) console.log(`  ${k.padEnd(7)} ${v}  vs raised ${n(contrast(v,'#FFFFFF'))}:1   ink ${TIMING.light[k]} on it ${n(contrast(TIMING.light[k], v))}:1`);
  console.log('\nlight wash resolution: accent-wash #FFE2EE with wash-ink candidates');
  for (const ink of ['#D1018A', '#A2006A', '#700148']) console.log(`  ${ink} on #FFE2EE = ${n(contrast(ink,'#FFE2EE'))}:1`);
  console.log('\nborder token: is signal-500 usable as an interactive boundary (floor 3.0)?');
  for (const s of ['#FE02A9', '#D1018A']) console.log(`  ${s}: light vs raised ${n(contrast(s,'#FFFFFF'))} canvas ${n(contrast(s,'#F7F8FB'))} sunken ${n(contrast(s,'#EFF1F5'))} | dark vs raised ${n(contrast(s,'#1A1C20'))} canvas ${n(contrast(s,'#0E0F13'))} sunken ${n(contrast(s,'#08090C'))}`);
}
if (mode === 'fix') fix();

function composite() {
  const over = (fg, bg, a) => { const F = hex2rgb(fg), B = hex2rgb(bg); return rgb2hex([0,1,2].map(i => a*F[i] + (1-a)*B[i])); };
  ...
  let l1 = over('#FE02A9', '#F7F8FB', 0.10);
  let l2 = over('#1B1E24', l1, 0.020);
  let lGlass = over('#FFFFFF', l2, 0.72);
  console.log(`light: canvas #F7F8FB -> orb ${l1} -> grain ${l2} -> glass ${lGlass}`);
  console.log(`       ink-primary #1B1E24 on ${lGlass} = ${n(contrast('#1B1E24', lGlass))}:1   ink-tertiary #6A6D74 = ${n(contrast('#6A6D74', lGlass))}:1   accent-ink #D1018A = ${n(contrast('#D1018A', lGlass))}:1`);
  ...
  console.log(`       ink-primary #F5F7F9 on ${dGlass} = ${n(contrast('#F5F7F9', dGlass))}:1   ink-tertiary #86898F = ${n(contrast('#86898F', dGlass))}:1   accent-ink #FE02A9 = ${n(contrast('#FE02A9', dGlass))}:1`);

  console.log('\n=== V-13  HERO TEXT OVER THE BACKGROUND FIELD (worst case) ===');
  console.log(`light: ink-primary on ${l2} = ${n(contrast('#1B1E24', l2))}:1   ink-secondary #53575E = ${n(contrast('#53575E', l2))}:1   accent-ink #D1018A = ${n(contrast('#D1018A', l2))}:1   accent-mark #FE02A9 = ${n(contrast('#FE02A9', l2))}:1`);
  console.log(`dark : ink-primary on ${d2} = ${n(contrast('#F5F7F9', d2))}:1   ink-secondary #B3B6BA = ${n(contrast('#B3B6BA', d2))}:1   accent-ink #FE02A9 = ${n(contrast('#FE02A9', d2))}:1`);
  console.log('\n=== V-14  GRID LINE / HAIRLINE visibility (decorative, no floor — recorded for the record) ===');
  console.log(`light grid rgb(27 30 36 / 0.05) over canvas = ${over('#1B1E24','#F7F8FB',0.05)}  ratio ${n(contrast(over('#1B1E24','#F7F8FB',0.05),'#F7F8FB'))}:1`);
  console.log(`dark  grid rgb(245 247 249 / 0.055) over canvas = ${over('#F5F7F9','#0E0F13',0.055)}  ratio ${n(contrast(over('#F5F7F9','#0E0F13',0.055),'#0E0F13'))}:1`);
  console.log('\n=== V-15  ACCENT vs the 12 brand colours — full CVD table (accent-mark #FE02A9) ===');
  for (const [t, hex] of Object.entries(BRAND)) {
    console.log(`  ${t.padEnd(13)} normal ${n(dE('#FE02A9', hex)).padStart(6)}  protan ${n(cvdDE('#FE02A9', hex, 'protan').worse).padStart(6)}  deutan ${n(cvdDE('#FE02A9', hex, 'deutan').worse).padStart(6)}  tritan ${n(cvdDE('#FE02A9', hex, 'tritan').worse).padStart(6)}`);
  }
  ...
  for (const theme of ['light','dark']) {
    ...
    for (const [nm, hx] of [...Object.entries(TIMING[theme]).map(([k,v])=>[`timing-${k}`,v]), ...Object.entries(STATUS[theme]).map(([k,v])=>[`status-${k}`,v])]) {
      console.log(`   ${nm.padEnd(16)} normal ${n(dE(acc,hx)).padStart(6)}  protan ${n(cvdDE(acc,hx,'protan').worse).padStart(6)}  deutan ${n(cvdDE(acc,hx,'deutan').worse).padStart(6)}  tritan ${n(cvdDE(acc,hx,'tritan').worse).padStart(6)}`);
    }
  }
}
...
function orb() {
  ...
  for (const a of [0.05,0.06,0.07,0.08,0.09,0.10,0.12]) {
    const f1 = over('#FE02A9','#F7F8FB',a);
    const f2 = over('#1B1E24', f1, 0.020);
    console.log(`  orb ${a.toFixed(2)} -> field ${f2}   accent-mark ${n(contrast('#FE02A9',f2))}:1   accent-ink ${n(contrast('#D1018A',f2))}:1   ink-tertiary ${n(contrast('#6A6D74',f2))}:1   border-control ${n(contrast('#878B92',f2))}:1`);
  }
  console.log('\n=== ORB OPACITY SOLVE — dark mode ===');
  for (const a of [0.14,0.17,0.20,0.24]) {
    const f1 = over('#FE02A9','#0E0F13',a);
    const f2 = over('#F5F7F9', f1, 0.038);
    console.log(`  orb ${a.toFixed(2)} -> field ${f2}   accent-mark ${n(contrast('#FE02A9',f2))}:1   ink-tertiary ${n(contrast('#86898F',f2))}:1   border-control ${n(contrast('#64686F',f2))}:1   ink-secondary ${n(contrast('#B3B6BA',f2))}:1`);
  }
}
if (mode === 'orb') orb();
...
function plate() {
  ...
  for (const [theme, canvas, orbA, grainC, grainA, tert, bctl, mark, ink2] of [
    ['light','#F7F8FB',0.09,'#1B1E24',0.020,'#6A6D74','#878B92','#FE02A9','#53575E'],
