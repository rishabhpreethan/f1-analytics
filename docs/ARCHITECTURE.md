...
| `GET /api/drivers/:reference` | ✅ **F4** — profile, career totals, season-by-season with every team of a split season, and one row per **race** (not per classification row) carrying grid, finish, qualifying position and places gained. DR-1 … DR-5 |
