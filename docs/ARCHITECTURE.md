...
| Charts | **No charting library.** Maths primitives — `d3-scale`, `d3-shape`, `d3-array` (all ISC, zero install scripts) — plus an in-repo SVG chart kit. **`d3-time-format` was the fourth and is uninstalled (§10 #31): two features shipped without importing it once.** **Recharts and visx are both rejected**, superseding §10 #3. | §10 #28, §4. Measured on this machine: our own Recharts import surface is **121.72 KB gz**, and even a five-component subset is **104.97 KB** against **89.75 KB** of remaining initial-JS headroom, because Recharts 3 has Redux Toolkit as a runtime dependency. visx is 29.96 KB and is `@visx/vendor` — the same d3 modules — plus React wrappers. The primitives are **16.33 KB**. Not installed until the first chart lands (F2). |
| Map data | **`world-atlas`, `topojson-client`, `topojson-simplify` — `devDependencies`, all ISC, none reaches the client.** They exist to run `scripts/generate-world-land.mjs`, which emits one string constant, `src/components/entity/worldLand.ts`. | §10 #35. The circuit atlas needs a coastline, and the alternative to generating it at build time is shipping a TopoJSON runtime and a 500 KB dataset to decode 12.9 KB of path. `npm audit` clean; no install scripts on any of the three (`prepublishOnly`/`postpublish` do not run on install); one transitive addition, `commander@2`, build-time only. Verified after the build that no shipped chunk contains `topojson`, `presimplify` or `world-atlas`. |
...
binary search come from three ISC d3 modules; everything that renders is ours, in
...
| Date tick formatting | ✗ **not used** | `d3-time-format` was carried for this and never imported: no axis in this product is a real date scale — rounds and laps are integers on a `scalePoint`. Uninstalled in F3 and now an ESLint error, because `d3-scale` keeps it resolvable (§10 #31). Dates go through `src/lib/format.ts`. |
...
```
/compare?e=max_verstappen,lando_norris
```

| Param | Meaning | Built in F7 |
|---|---|---|
| `e` | comma-separated `driver.reference` slugs, **max 4** (§4 direct-label rule) | ✅ |
| `kind` | `driver` \| `team` | **`driver` only.** The endpoint allowlists one value; `?kind=team` is a 400 rather than a driver comparison wearing the wrong label, and the enum gains its second member when the team lens exists |
| `from`, `to` | inclusive season range | ✗ — the season lens sends **every** season the selection entered in one response, so there is no range to narrow. See §6 |
| `metric`, `view` | | ✗ — replaced by the lens switch, which is local state on the page rather than URL state. See the note below |

**Two of the five parameters were never built, and that is a decision rather than a gap.** The lens
(`career` \| `season`) and the chosen year are `ComparePage`'s own state, not the URL's, because the
component that owns them publishes no callback for either. The consequence is stated plainly in
`src/routes/Compare.tsx`: a comparison addressed by a URL always works, and changing the selection
*from inside the page* does not yet reach the fetch. Lifting `selected`/`onSelect` and `year`/`onYear`
out of `ComparePage` is what closes it, and the route is written so that nothing else has to change.

Invalid params degrade to defaults with a visible notice — never a blank page, never a crash. On
`/compare` that means a malformed element of `?e=` is **dropped and the drop is announced**, where
the endpoint refuses the whole request; the two behaviours are deliberately different, because a
400 is a blank page (`src/features/compare/selection.ts`). With no `?e=` at all the page opens on a
**derived** default — the two most-raced drivers still on the current grid — written back into the
URL with `replace`.
...
| `GET /api/seasons/:year/races/:round` | ✅ **F3** — round + circuit metadata, the weekend's session list, the full classification, and **two availability flags**. No `lap` access beyond one short-circuiting `EXISTS`, so the reduced page is cheap |
| `GET /api/seasons/:year/races/:round/laps` | ✅ **F3** — per-driver lap traces (position + time, `isDeleted` **exposed, not filtered**) plus the session's pace summary. Fetched only when `availability.hasLapData` |
| `GET /api/seasons/:year/races/:round/stints` | ✅ **F3** — pit stops + derived stint boundaries + the duration distribution. Fetched only when `availability.hasPitData` |
| `GET /api/drivers` | ✅ **F7** — all **881** drivers: slug, name, `code` (null for 774), nationality, country code, the season span, and the **rankable career figures** — `races`, `starts`, `wins`, `podiums`, `championships`, `bestChampionshipPosition`. **No query parameters** — 22.6 KB gzipped, memoised, sorted and filtered client-side. `championships` is P1 in the **final** standing of a **complete** season, never a mid-season lead. **Gained `colorTeamRef` in F7** — most starts, ties to the most recent — because `/compare`'s picker has to make a bay recognisable before it is filled and cannot ask the comparison endpoint about a driver who is not in the comparison yet |
| `GET /api/drivers/:reference` | ✅ **F4** — profile, career totals, season-by-season with every team of a split season, and one row per **race** (not per classification row) carrying grid, finish, qualifying position and places gained. DR-1 … DR-5. ⚠ **`gridVsFinish`, `qualifyingVsRace.meanDelta`, per-race `positionsGained` and season `bestFinish` all changed value on 2026-08-23** — they were measured against `session_entry.position` on unclassified rows, which is a retirement order (`DATABASE.md` trap 27). Senna's mean places gained moved from −4.99 to −0.30 |
| `GET /api/teams` | ✅ **F7** — all **214** teams: slug, name, nationality, country code, the season span, and `races`, `wins`, `podiums`, `championships`, `bestChampionshipPosition`. 4.6 KB gzipped. Same conventions as the driver index; `podiums` counts **places** (a 1–2 is two) where the driver field counts races |
| `GET /api/teams/:reference` | ✅ **F5** — profile, honours, and one row per season with the constructors' standing, the lineup and the intra-team race-points split. CN-1 … CN-4 |
| `GET /api/circuits` | ✅ **F7** — all **78** circuits: slug, name, locality, country, country code, **latitude/longitude**, and **two** counts — `roundsHeld` (numbered rounds, including any not yet run) against `racesWithResults` — plus the year span and `lastScheduledYear`. 3.9 KB gzipped. Madring reads 1 and 0 with `lastScheduledYear` 2026, which is the field that says a venue is on the current calendar — `lastYear` cannot |
| `GET /api/circuits/:reference` | ✅ **F6 (data layer)** — venue profile with coordinates, every Grand Prix held there with its winners and pole sitters, and the most successful drivers and teams. CI-1 … CI-3. **The surface is not built yet**, so the route still renders a placeholder |
| `GET /api/compare` | ✅ **F7** — the career lens. 1–4 driver slugs in `?e=`: career totals, the season-by-season record with **whether each championship placing is final**, the career teammate ledger, every pairwise head-to-head, and a **lineage chain** for each pair who were not teammates. 24.6 KB raw / **2.9 KB gzipped** for four drivers, p50 **42 ms**. Gained `gridVsFinish` per entity 2026-08-23 — places made up from the grid over the career, with its denominator and its exclusions. **The identical object `GET /api/drivers/:reference` publishes, from the identical builder**, and `queries/compare.test.ts` asserts the two equal on four careers; no new statement and no new parameter |
| `GET /api/compare/seasons` | ✅ **F7** — the season lens. **Every season the selection entered, in one response**, round by round: cumulative championship points and placing, finishing position, the other seat in each principal's car with its occupant per round, and the season's best-N counting rule. 29 seasons for four drivers is 126.7 KB raw / **8.7 KB gzipped**, p50 **38 ms**. One response rather than one per year because `ComparePage` owns the chosen year and does not publish it — and because it makes the year rail cost no network at all (§8) |
...
- ~~Lap endpoints support optional `?drivers=` and `?fromLap=&toLap=` narrowing.~~ **Not
  implemented, deliberately — see convention 5 and §10 #30.** The race-lap endpoint takes no
...
4. **In-process memoisation is for payloads that are global, small and requested on every
   ...
   measurement, not an intuition. **The three race endpoints are not memoised either**, for the
   same reason and more strongly: the key space is every race in 77 seasons and the lap payload is
   the largest in the product.

6. **A `:reference` parameter is an allowlisted character class and a bounded length, and the
   class is measured rather than guessed** (added F4). `referenceParamSchema` is
   `^[A-Za-z0-9_-]{1,32}$` — **not** lowercase-only, which is the pattern that reads as
   obviously right: three driver references in the archive carry a capital (`scott_Brown`,
   `Changy`, `Cannoc`) and would have answered 400 for three real drivers, on URLs nobody types
   by hand. Hyphens are real too (`campbell-jones`, `brabham-alfa_romeo`). The length bound is
   the **format's** 32 against a measured maximum of 20, so convention 2 holds: a well-formed
   reference the dataset does not hold is a 404.

5. **A bound that the parameters already guarantee beats a narrowing parameter** (added F3,
   §10 #30). The race-lap endpoint declines the `?drivers=` and `?fromLap=` narrowing this section
   used to promise, on two grounds. **It is not needed:** `(year, round)` addresses exactly one
   race session, each round has exactly one, and the largest in the archive holds 1,649 lap rows
   at 1.45 ms — so the result set is bounded by construction and S-10 is satisfied without a
   parameter to validate. **And it would cost something real:** RD-1 plots the whole field, so the
   client fetches every lap regardless, and a filter parameter would turn RD-2's ≤ 4-driver
   selection into a network round-trip — breaking §8's "chart interaction < 100 ms, no network" to
   re-fetch rows already in the cache. A driver-profile endpoint spanning a whole season is where
   these parameters earn their place, and that is the amendment to make when it lands.
...
| API p95 for a lap endpoint | < 200 ms | nothing automated. **Measured in F3: 1.45 ms** for the largest race in the archive (2010 R18, 1,649 lap rows), warm prepared statement, 30 runs — `server/queries/race.ts`. That is the SQL, not the whole request, and it is not a p95; it is the figure that says the budget is not the constraint here |
...
**Techniques**
- ✅ **Route-level code splitting — landed 2026-08-23 (§10 #36).** Ten of the twelve route
  surfaces are `React.lazy`; `RootLayout`, `Landing` and `NotFound` are eager. Initial JS
  **225.76 → 161.28 KB gzipped**. The chart kit is now a shared chunk fetched by the six routes
  that draw, and `worldLand.ts` — 12.9 KB of coastline for one component — rides in the
  `/circuits` chunk alone. Both were verified against the emitted chunks, not inferred.
...
│   ├── routes/              one module per resource (meta, seasons, races, entities —
│   │                        the last holds the six drivers/teams/circuits routes, which
│   │                        are two handlers three times: an index and a profile)
...
│   ├── features/            feature hooks + pure selectors (meta, landing, season, race).
│   │                        `race/` is selectors.ts (states, classification) + pace.ts
│   │                        (the axis ceiling, clean laps, the fit, the inferred SC) +
│   │                        series.ts (chart-ready series) + useRace.ts
│   ├── components/
│   │   ├── layout/          shell: CommandDock, AtmosphereField
│   │   ├── ui/
│   │   │   └── icons.tsx    eleven inline SVGs (§10 #18) — the only icon module
│   │   ├── entity/         driver/team/circuit surfaces — the designer's, except
│   │   │                    worldLand.ts, which is **generated** (§10 #35) and never edited
...
├── scripts/                 repo tooling, ESM run by Node directly. **The three gates are
│   │                        zero-dependency; the one generator is not** — see §10 #35
...
│   ├── generate-world-land.mjs  emits src/components/entity/worldLand.ts from Natural Earth
│   │                        110m. The only script with imports, and they are all
│   │                        devDependencies (§2, §10 #35). stdout-only, like the palette
│   │                        emitter: `node scripts/generate-world-land.mjs > <target>`
│   └── world-land.test.mjs  byte-identity against a fresh run, plus the gzip ceiling —
│                            it needs node:zlib, which is why it is not under src/
...
| 28 | 2026-08-07 | **No charting library. The substrate is ~~four~~ **three** ISC d3 modules (`d3-time-format` removed by #31) — `d3-scale`, `d3-shape`, `d3-array`, `d3-time-format` — plus a chart kit written in this repository. Recharts and visx are both rejected, superseding #3.** ~~Nothing is installed yet; the four land with the first chart in F2.~~ — **installed in F2, 2026-08-07; see #29 for the re-measurement and the lint chokepoint.** | Rishabh's steer was to prefer primitives, on the grounds that this product's constraints will fight an opinionated library. Vetted rather than assumed, in an isolated scratch project so the repository's `node_modules` and lockfile were untouched; every figure below is `esbuild --bundle --minify --format=esm` with `react`/`react-dom` external, gzipped with Node zlib level 9 — the same method and the same encoder the §8 budget gate uses (#27), so these numbers are comparable to the build's. **Measured cost.** Recharts 3.10.1, our realistic 13-component import surface: 420,511 raw / **121,723 gz**. A minimal five-component surface (`LineChart`, `Line`, `XAxis`, `YAxis`, `Tooltip`) is still 353,961 / **104,971**, and Recharts declares `sideEffects: false`, so that is *after* tree-shaking — the floor is set by Recharts 3 having `@reduxjs/toolkit`, `react-redux`, `immer` and `reselect` as **runtime** dependencies. visx, a comparable surface across `@visx/{scale,shape,axis,group,tooltip,event,curve}`: 86,133 / **29,964**. The four d3 modules with a wide surface: 45,975 / **16,330**; `d3-scale` + `d3-shape` alone: 39,670 / **14,375**. Against the initial-JS budget the build stands at 160.25 KB of 250 KB, i.e. **89.75 KB of headroom** — so a minimal Recharts import exceeds the entire remaining budget before one chart is written. Charts will be route-split, so this is not literally a budget breach; it is ~105 KB gz per chart-bearing route, on the surface whose FCP target is 1.5 s, for capability we would then constrain heavily. **Supply chain (S-7/S-14).** d3 primitives: 10 packages, **all ISC**, **no install scripts anywhere in the tree**, all `sideEffects: false` bar two narrow locale entries, `npm audit` 0 vulnerabilities. They ship no types, so `@types/d3-{scale,shape,array,time-format}` (MIT, DefinitelyTyped, 6 packages, zero runtime cost) become devDependencies. Recharts: 42 packages, MIT, a `prepare: husky \|\| true` that npm does not run for a registry tarball, audit clean. visx: 47 packages, MIT, and `@visx/axis` → `@visx/text` → **`reduce-css-calc@1.3.0` → `math-expression-evaluator@1.4.0`** — a 2016-era expression evaluator reached for axis-label text measurement. **Why not visx, the obvious middle ground:** `@visx/vendor` is a re-export of the *same* d3 modules, with the `@types/d3-*` packages declared as **runtime** dependencies, and it pulls `d3-geo` and `d3-delaunay` we do not use. It costs 13.6 KB gz more than the primitives for React wrappers around components we must author anyway, because `DESIGN_SYSTEM.md` §6 requires one shared tooltip, token-driven colour rather than library defaults, a table view per chart and a designed dark mode. A single `@visx/*` package for a specific named need remains available as an amendment here. **Why not hand-rolled maths as well** (the most primitive option, 0 KB): `d3-array`'s tick-step selection and `d3-time`'s tick choice across day/month/year boundaries are the subtle part, and an axis spanning 1950–2026 with badly chosen ticks is a visible defect. The maths is not where this product's value is. Also rejected without measurement, each for a stated reason: **Chart.js / ECharts / Nivo / Observable Plot** — all more opinionated than Recharts and larger, and Chart.js and ECharts are dual-axis-first APIs, which is the one thing #6 and §4 forbid outright. **The decisive argument is not bytes, it is enforceability.** With no review gate (#25), §4's constraints otherwise rely on whoever writes the chart remembering them. Our own frame type takes exactly **one** y-scale, so a dual-axis chart is not expressible, where `<YAxis yAxisId="right">` is one line away in Recharts; series colour resolves from entity identity through `lib/teamColor.ts` with no index-into-a-palette path, so colour cannot follow rank; and a series without a non-colour channel does not compile, which is #6's mandatory secondary encoding turned into a type error. **Cost, stated rather than buried:** roughly an axis, a legend, a tooltip and a responsive wrapper move out of `node_modules` into this repository, and since they are presentational that work lands on the **`designer`** (§4). **What is not verified:** whether SVG at ~1,400 lap marks holds §8's <100 ms interaction budget. Paint and hit-test cost are not observable without a browser and there is none in this pipeline, so **no Canvas threshold is stated** — inventing one would be exactly the false precision #26 was written to correct. A line series renders one `<path>` per driver rather than one node per point, which is the reason to expect SVG to hold; moving a chart to Canvas is an amendment here carrying the measurement that justified it. |
| 29 | 2026-08-07 | **The four d3 primitives are installed (F2)** — *`d3-time-format` uninstalled again by #31, having never been imported*. **(F2), and `DESIGN_SYSTEM.md` §6's forbidden list becomes an ESLint error rather than a convention.** `d3-scale@4.0.2`, `d3-shape@3.2.0`, `d3-array@3.2.4`, `d3-time-format@4.1.0` as dependencies; `@types/d3-{scale,shape,array,time-format}` as devDependencies. `d3`, `d3-axis`, `d3-selection`, `d3-transition` and `d3-format` are added to `no-restricted-imports` for all of `src/**`. | #28 decided this on figures measured in an isolated scratch project. **Re-measured here, against the tree that actually shipped**, with #28's own method (`esbuild --bundle --minify --format=esm`, Node zlib level 9 — the encoder #27 makes the single authority): the wide surface §6.6 names is **45,568 raw / 16,299 gz = 16.30 KB**, reproducing #28's recorded 16,330 bytes to within 31 bytes, so the decision rests on a figure that has now been produced twice by two routes. **F2's own import surface is smaller — `scaleLinear`, `scalePoint`, `line`, `area`, `curveMonotoneX`, `extent`, `max`, `bisector`: 29,555 raw / 11,198 gz = 11.20 KB** — and charts are route-split, so the initial-JS bucket does not pay for it at all until a chart is on the first-paint path. **Supply chain re-verified on the installed tree, not on the registry metadata:** 10 packages resolved, **all ISC**, `sideEffects: false` on eight with `./src/defaultLocale.js` on `d3-format` and `d3-time-format`, and **zero `preinstall`/`install`/`postinstall`/`prepare`/`prepack` scripts anywhere in the subtree** — the `prepublishOnly`/`postpublish` entries `d3-scale` carries are publish-time and npm never runs them for a registry tarball. Types: 6 packages, **all MIT**, no scripts, no runtime cost. `npm audit` → `found 0 vulnerabilities` at 337 packages. **Why the lint rule and not a note:** §6 forbids `d3-axis`, `d3-selection`, `d3-transition` and `d3-format`, and with no review gate (#25) a forbidden import would otherwise be caught only by someone remembering the list. `d3-format` is the sharp case — it is **already in `node_modules`** as a transitive of `d3-scale`, so it resolves, type-checks and works, and the only thing standing between it and `src/lib/format.ts` drifting on lap-time formatting is a rule that fires. The `d3` meta-package is restricted too, though §6 does not name it: one `import { scaleLinear } from 'd3'` pulls `d3-geo`, `d3-delaunay` and the rest into the graph and defeats every figure above. The `src/lib/motion/**` block that lifts the gsap chokepoint **re-states** these five rather than switching `no-restricted-imports` off, because ESLint replaces a rule's options wholesale. **Not verified:** nothing here says the primitives are *sufficient* for the kit — that is the `designer`'s to find out while building it, and a fifth module would be an amendment on this line. |
| 30 | 2026-08-07 | **The race page is three endpoints, not one, and the lap-scale two are fetched only when the first says there is something to fetch. The axis-ceiling *rule* stays in the client; the *fact* it applies is stated by the server.** No `?drivers=` / `?fromLap=` narrowing (§6 convention 5, amending §6's own bullet). | **The reduced page is the common case, and that is the measurement the whole shape rests on: 484 races predate 1990 and exactly zero of them hold a single `lap` row.** So for most of the addressable pages the only thing there is to return is a classification and a grid, and a payload that shipped 1,649 lap rows to say "none" would make the majority case pay the flagship case's cost. The spine therefore carries `availability.hasLapData` / `hasPitData` — tested for `lap` rows, never `session.has_time_data` (trap 1) — and costs 0.22 ms with no `lap` access beyond one short-circuiting `EXISTS`. **The gating is structural, not documented:** `useRaceLaps` and `useRaceStints` take the `Race` payload rather than a year and a round, so the flag lives on the object the hook already needs and there is no way to call them for 1988 and have them fire. A `useRaceLaps(year, round)` signature would have made that the caller's job every time, forever. **One query, not two, for the lap payload.** `SQL_RACE_LAPS` returns every lap row for the race and one pure builder produces both the traces and the pace summary, so they cannot disagree about the session's fastest lap — a stronger guarantee than two statements agreeing by rule, and the reason the fastest lap can be server-stated at all. It is found as `min(time_ms)` over `is_deleted = 0` and **not** from `fastest_lap_rank`, which is absent on 133 race sessions that have lap rows and disagrees with the lap table on 5 more (`DATABASE.md` trap 18, §6.4a). **Where the ×1.5 lives, and why it is split.** The server states the *fact* (`pace.fastest`) because it is a pace metric — trap 8 belongs in the query layer — and because it is a property of the **session**, not of the selection: a ceiling derived from RD-2's four drivers would move when the fourth is toggled, so one race would show two axes on its two lap charts. The client applies the *rule* (`paceCeilingMs`, one exported function) because §6.3's 1.5 is a design decision about reading a chart; in the payload, changing it would be a server deploy and there would be two authorities on one number the moment §6.3 moved. Drift is prevented by there being exactly one function, not by moving the arithmetic across the wire. **Rejected:** one combined endpoint (the 1988 case carries two empty arrays and every reader of a classification pays 67 KB of laps); `?drivers=` narrowing (§6 convention 5 — the bound is already guaranteed by `(year, round)`, and the parameter would put a network round-trip inside a chart interaction); computing the pace summary in a second aggregate query (two derivations of one number, which is the thing this entry exists to avoid). **Not verified:** whether 22 lines × 58 laps of SVG holds §8's interaction budget — that needs a browser and #28 already declines to invent a threshold. |
| 31 | 2026-08-07 | **`d3-time-format` and `@types/d3-time-format` are uninstalled, and `d3-time-format` is added to `no-restricted-imports`.** The substrate is **three** d3 modules — `d3-scale`, `d3-shape`, `d3-array` — amending #28 and #29's four. | It had **zero import sites** across `src/` after both F2 and F3, measured by grep against `d3-array` 1, `d3-scale` 3, `d3-shape` 2. Nothing in this product plots a real date axis: the season hub's x-axis is a round number and the race page's is a lap number, both integers on a `scalePoint`, and the one place a date is rendered goes through `src/lib/format.ts`'s `formatIsoDate` because §2.4 requires one spelling of a date. So it was carried on an assumption from #28's planning — that a chart kit needs a time-axis formatter — that two features have now falsified. **The lint rule is the load-bearing half**, for exactly the reason #29 gave about `d3-format`: `d3-scale` declares `d3-time-format` as a dependency, so it stays **resolvable** after being uninstalled and would type-check and work. Removing the dependency without the rule would leave the package importable and the `package.json` misleading about what this product depends on. **Cost: 0 KB today**, since nothing imported it — this is dependency hygiene (S-7, S-14) rather than a budget change, and it is worth doing precisely because an unused dependency is invisible in a bundle measurement and visible in an audit surface. `npm audit` → `found 0 vulnerabilities` after removal. If a real date axis is ever needed, reinstate it with an entry here rather than importing what happens to resolve. |
| 32 | 2026-08-08 | **A query anchored on an *entity* re-derives the `driver`/`team`/`circuit` → `session_entry` spine in a `MATERIALIZED` CTE rather than filtering `v_race`. §3's "no feature re-derives a join path" is amended to "no feature re-derives a join path it can be entered through".** | The canonical views (#7) are entered by `(year, round)` and cannot be entered by an entity: `v_entry` starts at `session_entry` and reaches it through `session`, so a `driver_ref` predicate can only be applied **after** every row is produced. Measured on this machine, both shapes returning the same columns and the same rows, 30 timed runs warm: `alonso` (438 races, the deepest career) — `v_race WHERE driver_ref = ?` plans `SEARCH ses USING idx_session_type` and takes **20–23 ms p50**, the CTE plans `idx_driver_ref → idx_td_driver → idx_re_td → idx_se_re` and takes **5–8 ms**; `ferrari` (2,500 entries, the largest team) — **24–36 ms** against **13–17 ms**. A third shape was tried and is worse than both: resolving entry ids in a CTE and joining them back to `v_race` still enumerates every race entry and adds a bloom filter. **What is re-derived is exactly four joins** — the entity → `team_driver` → `round_entry` → `session_entry` spine — and everything below `session_entry` (round, season, circuit, team, driver) is joined the way `v_entry` joins it, so the *shape* of a classification row is still defined once. **`MATERIALIZED` is load-bearing, not decoration:** without it SQLite inlines the CTE and re-plans back to the view's shape. **`CROSS JOIN` is the second tool and it is a join-order hint, not a join kind** — SQLite forbids reordering across it. `SQL_CIRCUIT_RESULTS` needs it (without, the planner walked all 1,173 race sessions and checked the venue afterwards: 5.0 ms → 3.1 ms with the venue first) and `SQL_CIRCUIT_POLES` must **not** have it (forcing venue-first made it consult `idx_se_position` once per qualifying session: 1.2 ms → 74 ms). Both plans are asserted in `queries/circuits.test.ts`, and the driver and team plans in their own test files, because every one of these shapes returns **identical data** and only some of them are anchored on the parameter — a regression here is invisible except as latency. **What this does not license:** re-deriving a join path for a query that *can* be entered through the view, and inventing a third view. **Not verified:** nothing here says the anchored plan is optimal, only that it is anchored and faster; the figures move ~30 % between runs of the same process, so the ratio is what the decision rests on. |
| 33 | 2026-08-08 | **The three entity index endpoints list the *whole* archive — every driver, team and circuit — rather than only those that raced, and they take no query parameters at all.** | **Who is listed.** 63 of 881 drivers hold no race classification row, and they are two groups: **47 entered Grands Prix and never qualified** (Bernie Ecclestone 1958, Giovanna Amati 1992, Divina Galica, Desiré Wilson, and Claudio Langes, whose 14 entries in 1990 produced 14 failures to pre-qualify) and **16 hold FP1 rows only** — 2025–26 reserve drivers (Colton Herta, Felipe Drugovich, Paul Aron and thirteen more). 9 of 214 teams are the same case (Life, Eagle — `eagle` holds no `team_driver` row at all), and 1 of 78 circuits has a numbered round with no results (**Madring, 2026 R14** — trap 13, scheduled rather than missing). Filtering to `races > 0` was the alternative and was rejected on four grounds: **the index must enumerate exactly what the profile endpoint serves**, and `/api/drivers/ecclestone` answers 200 today — two endpoints disagreeing about what a driver is; **the product already prints “881 drivers”** to readers in `DriverPage`'s not-found copy, so a directory of 818 is an inconsistency a reader can see; **excluding is not reversible by the client and including is**, so the editorial decision stays where a human can change it; and it would **go stale by design**, because the 16 are current. The duty that creates is discharged in the payload shape: `races` (or `racesWithResults`) is on every row and the season span is null **exactly** when it is 0 — asserted on all 1,173 rows against the live database — so a client can tell an entered-only entity from a racing one *before* the click, without a second request. **Why no parameters.** The largest payload is **18.7 KB gzipped** (881 rows, 139 KB raw), immutable between refreshes and cached for an hour, so the client holds the whole directory and sorts or filters it in memory. A `?sort=` would add an allowlist to keep correct, a cache-key dimension, and a round-trip per click — and it would still be **wrong**, because SQLite compares text with BINARY collation and sorts `Räikkönen` after `Ryan`; the reader-facing sort needs `Intl.Collator` and therefore lives in `src/features/entity/selectors.ts`. The SQL's order is a stable default, nothing more. This is the counterpart to convention 5: a bound the *absence* of parameters guarantees beats a parameter to validate. **Memoisation.** All three are memoised for an hour, and this is convention 4's case rather than an exception — no parameter means a key space of exactly **one per endpoint**, ~180 KB retained in total, against 20.2 ms / 17.7 ms / 1.8 ms of aggregation per request (p50, 12 warm runs). **Not verified:** no surface renders these yet, so nothing about how a directory of 881 rows behaves in a browser has been observed. |
| 34 | 2026-08-23 | **The three index payloads carry achievement figures — `wins`, `podiums`, `championships`, `bestChampionshipPosition` (drivers and teams), `starts` (drivers), `latitude`/`longitude`/`lastScheduledYear` (circuits). This reverses the “a directory is not a dashboard” half of entry 33.** | **Why the reversal.** Entry 33's payload gave a client a name, a race count and a season span, so the only page the data permitted was a search box — and Rishabh rejected exactly that on 2026-08-23, contrasting it with the season hub, which is *“different, it has meaningful data, it's intuitive”*. Nothing was wrong with the reasoning; the result was undesignable. **A directory has to be rankable, stratifiable and groupable, and the API is where that becomes possible.** The rest of entry 33 stands unchanged: the whole archive is still listed, there are still no query parameters, and `races === 0` is still the field that separates the 63 drivers and 9 teams that never raced. **What it costs.** Four to five integers per row, no nested object: drivers 18.7 → **22.6 KB gzipped** (881 rows), teams 3.9 → **4.6 KB**, circuits 2.8 → **3.9 KB**. Query cost, p50 warm: drivers 11.1 → **15.9 ms**, teams **12.3 ms**, circuits **0.9 ms**, plus **5.4 ms** and **2.2 ms** for the two championship-standings statements — all of it paid once an hour behind the memo. The rule going forward is **a scalar or nothing**; a per-season array on an index row would be a profile. **The trap, and why it is structural.** `driver_championship` holds a *per-round snapshot*, so `WHERE position = 1` finds every driver who ever **led** a championship — 66, where the answer is **35** (17 constructors' teams, not 18). Two gates, neither of them a remembered check: the SQL joins `last_snapshot` so only a season's final standing exists in the result set, and `foldChampionshipStandings` takes the season-completeness map as a **required argument** so an unfinished season yields nothing. On the present data that gate is live rather than theoretical — 2026 is 10 rounds into 22 and would otherwise award Antonelli a title and Mercedes a ninth. It reads the same memoised map as `isChampion` on both profiles, so index and profile cannot drift, and `queries/directory.test.ts` asserts the 35/17 counts and the whole-archive equality with the profile endpoints. **Rejected: poles.** Measured first — qualifying classifications exist for **3 of Senna's 161 races**, 192 of Schumacher's 308, and none at all for Clark or Fangio. A `poles` column would print 3 beside Senna on a page built to rank. The profile can carry it because it carries `racesWithQualifying` and the prose beside it; a list row cannot. Fastest laps rejected on the same grounds (trap 18). Points rejected on trap 4. **Rejected: an `isCurrent` boolean.** `lastSeason === meta.latestSeason.year` already answers it (22 drivers, 11 teams) through one implementation, `selectEntityActivity`; a second representation is a second thing to keep true after a refresh. Circuits use `lastScheduledYear` against the same year, because a venue's round can be scheduled and unrun. **Not verified:** no page renders any of these fields yet — the `designer` is rebuilding the three index surfaces in parallel — so nothing about how they read on screen has been observed. |
| 35 | 2026-08-23 | **The circuit atlas gets a coastline, generated at build time from Natural Earth 110m into one string constant. `world-atlas`, `topojson-client` and `topojson-simplify` are added as `devDependencies`; nothing new reaches the client.** `scripts/generate-world-land.mjs` emits `src/components/entity/worldLand.ts`. | **§7.11's earlier ruling — a landmass "costs more than the entire chart kit" — was an estimate, and the number contradicts it.** Measured, gzipped at level 9, projected through the atlas's own identity map (`x = longitude + 180`, `y = 90 − latitude`): the naive 110m outline is **19.91 KB** and genuinely unaffordable, but at a simplification threshold of 1 on a 0.1° grid it is **5.01 KB** — 72 rings, 1,214 points, 12,919 bytes raw. Rishabh approved that spend. Wired into `CircuitAtlas` and rebuilt, initial JS goes **219.78 → 225.73 KB / 250 (87.9% → 90.3%)**, a marginal cost of **5.95 KB** — larger than the constant's standalone 5.01 KB because a 12.9 KB string compresses worse inside a 225 KB chunk of unrelated code than it does alone, which is the figure that actually matters and the one a standalone measurement understates by ~19%. ⚠ **`DESIGN_SYSTEM.md` §6.6.5.3 names the algorithm "Douglas–Peucker" and that is a mislabel.** Its recorded figures are `topojson-simplify`'s, which is **Visvalingam–Whyatt** — an *effective-area* rule, not a *perpendicular-distance* one. At the same numeric threshold the two are not interchangeable: a true DP at ε = 1.0° yields 744 points and 3.17 KB, a cheaper but visibly different map. The reproduced Visvalingam figures land within 6% of the recorded table at every threshold (1.0: 12.9 KB raw / 5.01 KB gz against a recorded 13.4 / 5.31; the residue is emit format — implicit linetos rather than explicit `L`), so **the numbers are right and the name is wrong**, and the picture the designer rasterised and signed off is Visvalingam's. The label is left for the designer to correct in its own document; the generator states the discrepancy at the point where someone would otherwise "fix" it by swapping the algorithm. **`d3-geo` is deliberately not a dependency** — the projection is two additions written out in the generator, because a `geoEquirectangular()` object carries scale, translate, rotate and clip extent, any of which moving would put the outline out of register with the 78 pips that `CircuitAtlas` places with that arithmetic inline. **Rejected: a static SVG in `public/`** — a second request, and it cannot follow the theme token. **Rejected: a CSS `background-image`** — the same bytes land in the render-blocking CSS bucket, which has 6.9 KB of headroom and would go to 93.6%. **Vetting:** all three packages ISC, no install scripts, one transitive addition (`commander@2`), `npm audit` unchanged — the single pre-existing high advisory is `nanoid` via `vite → postcss`, present on `main`, build-time only, and being fixed separately. `grep` over `dist/` after the build finds no `topojson`, `presimplify` or `world-atlas`. **Not verified: anything visual.** No agent can screenshot, and the constant is not rendered by anything at this commit — it tree-shakes away, so the committed tree still builds at 219.78 KB. Whether the outline registers with the pips, whether the one hole (the Caspian) punches out under `fill-rule: nonzero`, and whether the coastline reads at 517 px all need a browser. |
| 36 | 2026-08-23 | **Route-level code splitting lands: ten of the twelve route surfaces are `React.lazy`, with one Suspense boundary in `RootLayout` around the `Outlet`. `RootLayout`, `Landing` and `NotFound` stay eager.** Measured with `npm run check:budget`: initial JS **225.76 KB → 161.28 KB gzipped**, 90.3 % → 64.5 % of the 250 KB budget. | §8's own basis said the honest answer at 90 % is splitting, not a bigger number, and F7 needed the room. **What was actually in the initial chunk, read off the emitted chunks rather than reasoned about** (a `--sourcemap` build, `sources` per chunk): the whole chart kit plus its d3 modules, and `worldLand.ts` — 12.9 KB of coastline used by one component on one route. Both are now behind a dynamic import: the kit is a shared chunk (21.14 KB gz) fetched by the six routes that draw, the coastline rides in `CircuitIndex` alone. **The gate needed no change** — §8.1 defined "initial" as what the built `index.html` tells the browser to fetch, and Vite emits no `modulepreload` for a dynamically imported chunk, so the derivation was already right; verified on the built HTML, which now references exactly one JS file. Per-route incremental cost, gzipped: `/compare` and `/records` 0.67 · `/drivers` and `/teams` 12.15 · `/circuits` 18.29 · `/seasons` 32.70 · `/circuits/:ref` 32.22 · `/teams/:ref` 32.97 · `/seasons/:year/races/:round` 32.84 · `/drivers/:ref` 34.04 — so the **worst** first navigation, 195.3 KB, is still below the pre-split 225.76 KB every visitor used to pay. **Why the three eager ones:** the layout is on every URL and owns the boundary; `/` is the entry point for a visitor with no link; `*` answers a dead URL and costs ~0 bytes because the shell already carries `StateCard`, `ButtonLink` and the icons. **Why one stable boundary:** React does not show a fallback for a boundary already displaying content when the suspension happens inside a transition, and `BrowserRouter` wraps its location update in `startTransition` by default (verified in `react-router@8.3.0` source, `dist/production/lib/dom/lib.js`) — so an in-app navigation holds the current page until the chunk lands instead of flashing. A per-route boundary would be a *new* boundary each time and would flash. **Rejected:** `build.rolldownOptions.output.advancedChunks` hand-partitioning, because the dynamic-import graph already produced the split the routes imply and a hand-written chunk map is a second place for the route table to drift. **Not verified here:** anything about how the transition looks or feels — jsdom does no layout, compositing or timing. `src/App.test.tsx` pins the one edit that silently undoes the split (a static import of a lazy route module); the size claim is the build gate's, not a test's. |
| 37 | 2026-08-23 | **The comparison workspace is two endpoints, not one with a `lens` parameter: `GET /api/compare` and `GET /api/compare/seasons`.** | Their cache lifetimes are independent. Flipping the lens must not refetch a career payload that has not changed, and neither must a change of year — two routes are two TanStack Query keys and the client keeps both. Rejected: one endpoint returning both lenses, which couples them and doubles the cold cost of every interaction that touches either. |
| 38 | 2026-08-23 | **The season lens sends *every* season the selection entered in one response, rather than one year per request.** | `ComparePage` keeps the chosen year in local state and publishes no callback, so a per-year endpoint could not be told which year to ask for. Sending them all costs little and buys §8's "chart interaction < 100 ms, **no network**" on the year rail outright: measured at **29 seasons, 126.7 KB raw / 8.7 KB gzipped, p50 38 ms** for the four drivers the surface was designed against, and 164.8 KB / 12.0 KB for the four longest careers in the archive. It is bounded by the same parameter the career lens is — four careers, the longest 23 seasons — so the union cannot exceed the archive's 77 years. Revisit with a measurement if a future lens widens the selection. |
| 39 | 2026-08-23 | **The teammate graph joins two drivers only when they shared a *race*, and the chain search minimises unmeasured links before hops.** | The roster graph — anyone against the same team in the same season — over-claims: **38.6 % of its edges never shared a race**, and Coulthard and Senna are the proof, both Williams drivers of 1994 who never once lined up together. The shared-race graph is **777 drivers, 4,678 pairings, 759 in one component**, and **2,419 of those pairings (51.7 %) are unmeasured** — the two never both finished. So a hop-minimal chain is very often a chain of links that say nothing, and the Dijkstra weight is **1,001 for an unmeasured pairing against 1 for a measured one**: a total cost then decomposes exactly as `1000 × unmeasured + hops`, which sorts by both keys in one scalar. Ties break on the strongest weakest link, then total evidence, then `reference` — determinism is the requirement, because a chain that changed between two page loads would be worthless. Rejected: BFS by hop count, which is what the fixture used and which routes Verstappen to Fangio through two pairings with no measured result in them. |
| 40 | 2026-08-23 | **`GET /api/drivers` gains `colorTeamRef`** — the team with the driver's most starts, ties to the most recent. | `/compare`'s picker has to make a bay recognisable *before* it is filled, and cannot ask the comparison endpoint about a driver who is not in the comparison yet. Added to the existing memoised index rather than published as a near-duplicate `/api/compare/directory`: the client already caches that payload for `/drivers`, so the picker is usually free. Cost is one window function over the `driver_race` CTE that was already being produced. A reference, never a hex — no colour crosses a schema boundary (`DESIGN_SYSTEM.md` §3.3a.3). |
| 41 | 2026-08-23 | **`session_entry.position` is never NULL on a race row, so every metric derived from it gates on `is_classified`.** Four career figures were corrected: `positionsGained`, `gridVsFinish`, `qualifyingVsRace.meanDelta` and season `bestFinish`. | The column is non-NULL on all 26,093 race rows and carries the order the cars *stopped* on the 9,683 unclassified ones — minimum 5, so it never looks wrong and no `null` check ever fires. `grid - position` therefore measured against a retirement order: Senna read **−4.99** mean places lost per race against an honest **−0.30** over the 108 races he was classified in, and the exclusion counter beside it read **0 for all 818 drivers with a race** while `DriverProgress` rendered it in a sentence. 236 tests were green throughout, because the fixture for a retirement used `position: null` — a row shape the database has never produced. Recorded as `DATABASE.md` trap 27; §2.3's own column note said "NULL when not classified" and is corrected. Win and podium counts are **not** changed and were verified safe: no unclassified row carries a position under 5. Left unfixed and named rather than silent: the same `min(position)` shape in `queries/teams.ts` (twice) and `queries/circuits.ts` (once), neither of which reaches a screen today. |
| 42 | 2026-08-23 | **`GET /api/compare`'s career lens publishes `gridVsFinish` — the driver profile's object, from the driver profile's builder**, rather than a compare-shaped statistic of its own. `gridVsFinishSchema` moved to `schemas/entity.ts`; `buildGridVsFinish` takes a narrow `GridVsFinishRace` instead of a `DriverRace`. | The metric already existed on `/api/drivers/:reference`, so the only question was whether the two agree. A parallel definition would have been two answers to "how many places did Hamilton make up", and this module's stated contract is that its `totals` equal the profile's; extending that contract cost one widened parameter and one field. **No new SQL statement, no new parameter and no new whole-relation read** — everything is derived from the `CollapsedRace[]` the lens already builds, so S-4 and S-10 are unchanged. The schema moved to `entity.ts` rather than being imported from `driver.ts` so `schemas/compare.ts` does not pull the 400-line driver module into the browser bundle. **Career only:** a per-season figure sits on 14–22 counted races in a modern season and 2–8 in a 1950s one, and Hamilton's season means swing −1.67 to +3.25 against medians that stay at 0 — a noise series nobody asked for. **Counts published beside the mean**, because at ≥20 counted races Fangio and Clark are careers where the two disagree in sign. **Median rejected as the headline:** measured across an era-spanning panel it takes only the values {−1, 0, 1, 2} and is 0 for six of eleven, so a diverging bar drawn on it encodes almost nothing. |
