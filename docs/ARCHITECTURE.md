...
| `GET /api/drivers` | ✅ **F7** — all **881** drivers: slug, name, `code` (null for 774), nationality, country code, distinct races entered and the season span. **No query parameters** — 18.7 KB gzipped, memoised, sorted and filtered client-side |
...
| `GET /api/teams` | ✅ **F7** — all **214** teams: slug, name, nationality, country code, races entered and the season span. Same shape and same conventions as the driver index |
...
| `GET /api/circuits` | ✅ **F7** — all **78** circuits: slug, name, locality, country, country code, and **two** counts — `roundsHeld` (numbered rounds, including any not yet run) against `racesWithResults` — plus the year span. Madring reads 1 and 0 |
