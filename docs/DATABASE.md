...
**There are 21.** `CLAUDE.md` and `.claude/agents/*.md` still say "the 14 traps"; this table is the
authority. Trap 16 was found in F2 by querying rather than by reading, and **traps 17–21 were found
in F3**, the first feature to touch `lap` and `pit_stop` in anger — every one of them by running a
query rather than by reading this document. The count in those other files is stale — flagged rather
than edited, because they are not this document's to change.
