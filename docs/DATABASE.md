...
| `position` | finishing position when classified. ⚠ **Never NULL on a race row** — on the 9,683 unclassified ones it is the order the cars *stopped*, minimum 5. Gate on `is_classified`; see trap 27, which this row's earlier wording caused |
...
**There are 27.** `CLAUDE.md` and `PLAN.md` were corrected to 25 on 2026-08-23 and need correcting again to **27**; `.claude/agents/*.md` may still say "the 14 traps". This table is the
...
career-length question, **trap 25 was found during the index redesign**, the first work to count titles across the
whole archive at once, **trap 26 was found in F7** while building the comparison workspace's
season lens, and **trap 27 was found by a shipped arithmetic defect** while extending that
workspace — every one of them by running a query rather than by reading this
...
| 3 | `position IS NULL` ≠ DNF | Use `is_classified` / `status` (§3). **On a race row it is stronger than that — see trap 27**: `position` is never NULL there at all, so the NULL test is not merely wrong, it is unreachable |
...
| 26 | **`driver_championship.season_id` is NULL on 91.3 % of rows, and the natural join through it fails _silently and plausibly_** | Counted: **32,963 of 36,091** driver-championship rows carry no `season_id`, and the same shape holds on `team_championship`. The obvious way to attach a snapshot to its season is `JOIN season s ON s.id = dc.season_id`, and it does not error, return nothing, or look wrong — it returns the **3,128 rows that do carry one**, which happen to be the seasons' *final* snapshots. So the query answers a different question with a plausible-looking result: it gave 36 champions on a first attempt, which is the right answer by luck, and would give a per-round progression with exactly one point per season. **`year`, `round_number` and `session_number` are the reliable columns** — they are denormalized precisely so no join is needed (§2.5) — and every canonical query in §6.5, §6.6 and §6.6a already uses them. The rule: **never join these two tables on `season_id`**, and never filter on it either. `server/queries/compare.test.ts` and `server/queries/compareSeason.test.ts` assert the absence of the string `season_id` in their statements, which is a blunt test and the right one — the failure mode is a join that works. |
| 27 | **`session_entry.position` is never NULL on a race row, so every `position IS NULL` guard is dead code — and on an unclassified row the value is a _retirement order_ with a plausible magnitude** | Trap 3 says `position IS NULL ≠ DNF`, which reads as advice about a NULL that turns up. It does not turn up: counted, `position` is non-NULL on **all 26,093 race rows**, 9,683 of them `is_classified = 0`. So a guard written `position === null` to mean "did not finish" never fires, and the branch behind it is unreachable — while the value that flows past it is the order the cars **stopped**, not a result. **The minimum position on an unclassified row is 5**, which is what makes this silent in both directions: it is small enough to look like a finishing place in any arithmetic, and large enough that `position <= 3` and `position = 1` can never see it, so win and podium counts are safe and every *difference* is not. Two career metrics shipped wrong on this: `grid - position` gave Senna **−4.99** mean places lost per race against an honest **−0.30** over the 108 races he was classified in, and the exclusion counter beside it (`gridVsFinish.excluded.unclassified`) read **0 for all 818 drivers with a race** while the driver page rendered it in a sentence. The rule: **`is_classified` gates every metric derived from `position`, and `position IS NULL` gates nothing.** Also live at the time of writing and *not* fixed with it — `bestFinish` derived from `min(position)`, which is contaminated on **648 driver-seasons** (`queries/teams.ts` twice, `queries/circuits.ts` once; `queries/drivers.ts` was corrected). `driver_championship.position` is a different column on a different table and **is** genuinely nullable — trap 25's rules apply there, not this one.
