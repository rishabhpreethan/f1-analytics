...
| Collision detection is **precomputed**, not measured in the browser | §6.4 asks for pairwise perceptual distance on the colours actually assigned. The palette is a **closed set of 64 plotting tokens**, so all 2016 distances are computed by the validator at build time and shipped as one bit per pair. Doing it literally would put CIEDE2000, two CVD models and a `getComputedStyle` read of every custom property in a chart component on every render — and would be **untestable**, because jsdom resolves no custom properties and every ladder assertion would have been vacuous. `DESIGN_SYSTEM.md` §6.4, §9.2.4. |
| A pair collides in **either** theme, not the current one | Measuring in the theme in force lets a sunset **withdraw** a dash pattern. The encoding has to be a property of the entities, which is the same argument §6.4a already makes for the teammate shade pair. |
| The ladder fires **chart-wide**, per series | Escalation is decided pairwise, exactly as §6.4 says; the resulting channel is then applied to every series. Two of four series carrying a marker shape reads as an accident rather than an encoding, and the legend would have to explain a distinction that applies to half its rows. |
| Chart markers are equal **area**, not equal width | A square and a circle of the same bounding box differ by ~27% to the eye, so a shared-width marker set encodes magnitude by accident on a channel that carries identity only. `DESIGN_SYSTEM.md` §6.3. |
